function rmr_crcnspvc8_getspace
...
nwaynmethod  = 'ncomp10'; %'ncomp15sr'; % 'splitrel'
nwaynrand    = 50; % 'rnd40_conv1e-6'
nwayconvcrit = 1e-8;
normmethod   = 'avgoverepoch_8throotpower'; % 'coh'   'avgoverepoch' 'propodtod' 'kthrootpower'
...
for    isess = 1%:-1:1     ;% :numel(info.session); % 1 session for now
  
  
  % set currs
  currsess = [info.session{isess} '_' trialdeftype];
  ...
  if ~exist(nwayfn,'file')
    ...
    cfg.checkpointpath     = [info.scratchpath 'CHECKPOINT' '_' 'pvc8' '_' currsess '_' nwaynmethod '_' normmethod '_'];
    % qsub options
    switch distcompsys
      case 'torque'
        cfg.distcomp.system          = 'torque';
        cfg.distcomp.timreq          = 60*60*.5; %
        cfg.distcomp.matlabcmd       = '/opt/matlab/2015a/bin/matlab';
        cfg.distcomp.torquequeue     = 'hotel';
        cfg.distcomp.torquestack     = 25;
...
% get info
info = rmr_crcnspvc8_info;
%info.datapath = '/Users/roemervandermeij/Work/Data/tmpdata/CRCNS_PVC8/';
%info.savepath = '/Users/roemer/Work/Data/tmpdata/crcnspvc8/';
...
% set cgname suffic
%cgnamesuffix = 'contcorrgram_-20to20ms_first20natonly_keeptrials';
cgnamesuffix = 'contcorrgram_-20to20ms_first270natonly_keeptrials';
...
nwaynrand    = 50; % 'rnd40_conv1e-6'
nwayconvcrit = 1e-8;
normmethod   = 'avgoverepoch_8throotpower_2ndpassC'; % 'coh'   'avgoverepoch' 'propodtod' 'kthrootpower'
nwaysplit    = 'oddevenspikes'; %  oddevenspikes  oddeventrials
nwayadd = [normmethod '_' nwayalg '_' nwaynmethod '_' 'rnd' num2str(nwaynrand) '_' 'conv' num2str(nwayconvcrit)];
...
for    isess = 1%:-1:1     ;% :numel(info.session); % 1 session for now
  ...
  if exist(nwayfn,'file')
    ...
    % read in data
    fsample = 20000;
    data = rmr_crcnspvc8_readspike([info.datapath info.session{isess}],fsample,trialdeftype);
...
%     data.trialinfo   = data.trialinfo(trialind,:);
    ...
    % select first 270
    selind = find(data.trialinfo(:,1) <= 270); %
    data.trial = data.trial(selind);
    data.time  = data.time(selind);
    data.sampleinfo  = data.sampleinfo(selind,:);
    data.trialinfo   = data.trialinfo(selind,:);
    
        
    % plot
    cfg = [];
    cfg.condind   = {nwaycomp.trialinfo(:,4)==1,nwaycomp.trialinfo(:,4)==2}; 
    cfg.condlabel = {'small','big'};
    cfg.cgfn      = cgfn;
