function rmr_crcnshc5_getspace
...
%info.savepath = '/Users/roemervandermeij/Work/Data/tmpdata/crcnshc5/';
...
% set cgname suffic
cgnamesuffix = 'mazerun_contcorrgram_-20to20ms_3segsel_keeptrials';
...
nwaynrand    = 50; % 'rnd40_conv1e-6'
nwayconvcrit = 1e-8;
normmethod   = '32throotpower'; % % 'coh'   'avgoverepoch' 'propodtod' 'kthrootpower'
...
for    isess = 1%:-1:1     ;% :numel(info.session); % 1 session for now
  ...
  if exist(nwayfn,'file')
    ...
    data = rmr_crcnshc5_readspike([info.datapath currsess],fsample,'mazerun',0,1,1);
    ...
    % plot
    cfg = [];
    cfg.condind   = {nwaycomp.trialinfo(:,3)==1,nwaycomp.trialinfo(:,3)==2};
    cfg.condlabel = {'R','L'};
    cfg.cgfn      = cgfn;
    cfg.cgramdisp = 'pasquale2008'; % pasquale2008 sum
    rmr_plotspacespike(cfg,nwaycomp,data);

    % title(['R vs L   t = ' num2str(stats.tstat,'%1.2f')   ',  corr pos ' num2str(corr(C(:,icomp),mean(nwaycomp.trialinfo(:,end-1:end),2)),'%1.2f')])
